  ...
  return (
    <section className="h-screen overflow-hidden relative flex w-screen ">
      ...
      >
        {slider.map((slide) => (
          <div
            className={`flex text-black ${slide.bg}  gap-8  px-20 py-16 w-screen h-full`}
            key={slide.title}
          >
            <div
            ...
            >
              <h1 className="font-mont font-bold text-5xl">
                {slide.title}
                <br />
                <span className="text-mainBlue">{slide.highlight}</span>
              </h1>
              <p className="mt-10 text-2xl pr-16 font-medium">
                {slide.text}
                {/* Desde <span className="font-bold">los Simpson</span> hasta{" "}
                <span className="font-bold">Dragon Ball</span>, el mejor regalo
                para un amigo y{" "}
                <span className="font-bold">
                  pronto vendran personalizadas!
                </span> */}
              </p>
...
{
  /* <div className="flex text-black bg-blue-100 gap-8  px-20 py-16">
        <div
          className={`w-1/2 ${!loaded && "opacity-0"} ${
            loaded && styles["slideDown"]
          }`}
